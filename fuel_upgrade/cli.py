...
def parse_args(args):
    ...
    # check the path to update file is not empty
    # for any systems except backup
    if 'backup' not in rv.systems and not rv.src:
        parser.error(
            'please, specify path to update file with --src option'
            'path to update file cannot be empty for %s'
            .format(', '.join(rv.systems))
        )
